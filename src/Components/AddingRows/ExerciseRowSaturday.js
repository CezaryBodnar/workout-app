...
const ExerciseRowSaturday = ({ index, SaturdayValues, setSaturdayValues, addExerciseSaturday, setAddExerciseSaturday, add }) => {



    const deleteRow = (index) => {
        setAddExerciseSaturday(addExerciseSaturday.filter(el => el.id !== add.id));
        const values = [...SaturdayValues];
        values.splice(index, 1);
        setSaturdayValues(values);
        localStorage.removeItem('saturday-exercises');
    }
...
export default ExerciseRowSaturday
